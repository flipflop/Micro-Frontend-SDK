...
function Router(_ref3) {
  ...
  true ? warning2(location != null, '<Router basename="' + basename + '"> is not able to match the URL ' + ('"' + pathname + search + hash2 + '" because it does not start with the ') + "basename, so the <Router> won't render anything.") : void 0;
  if (location == null) {
    return null;
  }
...
function getNewMatchesForLinks(page, nextMatches, currentMatches, location, mode) {
  ...
  let newMatches = mode === "data" && location.search !== path.search ? nextMatches.filter((match2, index) => {
    ...
    return (mode === "assets" || match2.route.hasLoader) && (isNew(match2, index) || matchPathChanged(match2, index));
  });
  return newMatches;
}
...
function createTransitionManager(init) {
  ...
  async function send(event) {
    switch (event.type) {
      ...
      case "fetcher": {
        ...
        if (fetchControllers.has(key))
          abortFetcher(key);
        let match2 = getFetcherRequestMatch(new URL(href, window.location.href), matches2);
  ...
  function getFetcherRequestMatch(url, matches2) {
    ...
    if (!isIndexRequestUrl(url) && match2.route.index) {
  ...
  async function handleActionFetchSubmission(key, submission, match2) {
    ...
    let results = await callLoaders(state, state.transition.location || state.location, matchesToLoad, controller.signal, maybeActionErrorResult, maybeActionCatchResult, submission, match2.route.id, loadFetcher);
    if (controller.signal.aborted) {
      return;
    }
    fetchReloadIds.delete(key);
...
function filterMatchesToLoad(state, location, matches, actionErrorResult, actionCatchResult, submission, submissionRouteId, fetcher, catchBoundaryId) {
  ...
  if (catchBoundaryId || submissionRouteId && (actionCatchResult || actionErrorResult)) {
    ...
    matches = matches.filter((match2) => {
      ...
      if (match2.route.id === submissionRouteId || match2.route.id === catchBoundaryId) {
  ...
  return matches.filter((match2, index, arr) => {
    ...
    return match2.route.loader && (isNew(match2, index) || matchPathChanged(match2, index) || ((_location$state2 = location.state) === null || _location$state2 === void 0 ? void 0 : _location$state2.setCookie));
  });
}
...
var useInsertionEffect2 = React6["useInsertionEffect"] ? React6["useInsertionEffect"] : function useInsertionEffect3(create) {
  create();
};
...
var useInsertionEffect5 = React7["useInsertionEffect"] ? React7["useInsertionEffect"] : import_react6.useLayoutEffect;
...
var useInsertionEffect7 = React15["useInsertionEffect"] ? React15["useInsertionEffect"] : function useInsertionEffect8(create) {
  create();
};
...
function createBox(options = {}) {
  ...
  const Box = /* @__PURE__ */ React18.forwardRef(function Box2(inProps, ref) {
...
function createContainer(options = {}) {
  ...
  const Container = /* @__PURE__ */ React19.forwardRef(function Container2(inProps, ref) {
    ...
    return /* @__PURE__ */ (0, import_jsx_runtime5.jsx)(ContainerRoot, _extends({
      as: component,
      ownerState,
...
function createPalette(palette2) {
  const {
    ...
    contrastThreshold = 3,
    tonalOffset = 0.2
  } = palette2, other = _objectWithoutPropertiesLoose2(palette2, _excluded11);
 ...
 * LICENSE file in the root directory of this source tree.
 */
//# sourceMappingURL=/build/_shared/chunk-DAJFFMJP.js.map
